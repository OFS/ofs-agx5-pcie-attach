...
#VLOG_OPT += -debug_all 
ifeq ($(PARTCMP),1)
VCS_OPT = -full64 -ntb_opts uvm-1.2 -licqueue  +vcs+lic+wait -l vcs.log -partcomp ofs-agx5_tb_lib.tb_top   -partcomp_dir=./libraries/iofs_partition_lib -partcomp
else
...
ifeq ($(PARTCMP),1)
	...
	echo ofs-agx5_top_lib:                   ./libraries/ofs-agx5_top_lib >> ../ip_libraries/synopsys_sim.setup
	echo ofs-agx5_rtl_lib:                   ./libraries/ofs-agx5_rtl_lib >> ../ip_libraries/synopsys_sim.setup 
	echo ofs-agx5_tb_lib:                    ./libraries/ofs-agx5_tb_lib >> ../ip_libraries/synopsys_sim.setup
endif
...
ifdef AFU_WITH_PIM
	...
	cd $(VERDIR)/sim && vlogan $(VLOG_OPT) +define+FIM_C +define+SIM_VIP -F $(SCRIPTS_DIR)/generated_rtl_flist.f $(AFU_FLIST_IMPORT) -work  ofs-agx5_rtl_lib -l vlog_rtl.log

vlog_adp_ss_lib: 
	cd $(VERDIR)/sim && vlogan -ntb_opts uvm-1.2 -sverilog
	cd $(VERDIR)/sim && vlogan -full64 -ntb_opts uvm-1.2 -sverilog -timescale=1ns/1ns
	cd $(VERDIR)/sim && vlogan $(VLOG_OPT) +define+SIM_VIP -F $(SCRIPTS_DIR)/ip_flist.f -work ofs-agx5_top_lib -l vlog_ss_lib.log

vlog_adp_verif: 
	cd $(VERDIR)/sim && vlogan -ntb_opts uvm-1.2 -sverilog
	cd $(VERDIR)/sim && vlogan -full64 -ntb_opts uvm-1.2 -sverilog -timescale=1ns/1ns
	cd $(VERDIR)/sim && vlogan $(VLOG_OPT) +define+FIM_C +define+SIM_VIP -F $(VERIF_SCRIPTS_DIR)/ver_list.f -work ofs-agx5_tb_lib -l vlog_verif.log

vlog_svt:  
	cd $(VERDIR)/sim && vlogan -ntb_opts uvm-1.2 -sverilog
	cd $(VERDIR)/sim && vlogan -full64 -ntb_opts uvm-1.2 -sverilog -timescale=1ns/1ns
	cd $(VERDIR)/sim && vlogan $(VLOG_OPT) +define+SIM_VIP -F $(VERIF_SCRIPTS_DIR)/svt_list.f -work ofs-agx5_tb_lib -l vlog_svt.log
...
ifdef RTILE_SIM
	...
	cd $(VERDIR)/sim && mkdir $(TEST_DIR) && cd $(TEST_DIR) &&  cp -f $(OFS_ROOTDIR)/ofs-common/src/common/fme_id_rom/fme_id.mif . && cp -f $(VERIF_SCRIPTS_DIR)/fme_id.ver . && cp -f $(VERIF_SCRIPTS_DIR)/recalibration.ver . && ../simv $(SIMV_OPT) $(SIMV_OPT_EXTRA)
else
	cd $(VERDIR)/sim && mkdir $(TEST_DIR) && cd $(TEST_DIR)  && cp -f $(OFS_ROOTDIR)/ofs-common/src/common/fme_id_rom/fme_id.mif . && cp -f $(VERIF_SCRIPTS_DIR)/fme_id.ver . && cp -f $(VERIF_SCRIPTS_DIR)/recalibration.ver .  && ../simv $(SIMV_OPT) $(SIMV_OPT_EXTRA)
endif
endif
